...
<div class="w-full space-y-6">
    ...
    <div class="flex items-center justify-between bg-white p-6 rounded-lg border border-slate-100 shadow-sm">
        <div class="flex items-center gap-6">
            ...
            <div>
                <h2 class="text-2xl font-semibold text-slate-800 tracking-tight leading-none">Edit & Reset Sandi Guru BK</h2>
                <p class="text-slate-400 text-xs text-slate-500 font-medium mt-2">Pembaruan Informasi: {{ $teacher->user->name }}</p>
            </div>
        </div>
    </div>
    <form action="{{ route('admin.teachers.update', $teacher->id) }}" method="POST" class="grid grid-cols-1 xl:grid-cols-12 gap-6">
            ...
            <div class="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden bg-white">
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="name" required oninput="this.value = this.value.replace(/[^a-zA-Z\s.,']/g, '')" value="{{ old('name', $teacher->user->name) }}" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition font-medium" placeholder="Masukkan nama lengkap & gelar">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" required value="{{ old('email', $teacher->user->email) }}" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition font-medium" placeholder="email.guru@gmail.com">
                    </div>

                    <div>
                        <label class="block text-[9px] font-semibold text-slate-400 font-medium mb-1.5 ml-1">NIP (Nomor Induk Pegawai)</label>
                        <input type="text" name="nip" maxlength="18" oninput="this.value = this.value.replace(/[^0-9]/g, '')" value="{{ old('nip', $teacher->nip) }}" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition font-medium" placeholder="18 Digit Angka NIP">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="max_quota" value="{{ old('max_quota', $teacher->max_quota) }}" min="1" required class="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition font-bold text-center">
                    </div>

                    <div class="md:col-span-2 border-t border-slate-100 pt-5" x-data="{ showPass: false }">
                        <label class="block text-[9px] font-semibold text-slate-400 font-medium mb-1.5 ml-1">Sandi Baru (Reset)</label>
                        <div class="relative">
                            <input :type="showPass ? 'text' : 'password'" name="password" class="w-full bg-slate-50 border border-slate-200 rounded-lg pl-4 pr-10 py-3 text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition font-medium" placeholder="Kosongkan jika tidak ingin mengubah kata sandi" value="{{ old('password') }}">
                        ...
                        <span class="text-[9px] font-bold text-slate-400 tracking-wide mt-1 block">Isi kolom ini jika ingin me-reset kata sandi Guru BK ini. Password harus minimal 8 karakter, mengandung huruf besar, huruf kecil, angka, dan karakter khusus (@$!%*?&).</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="xl:col-span-4">
            <div class="bg-white border border-slate-200 rounded-lg shadow-sm p-8 flex flex-col justify-center items-center text-center relative overflow-hidden group">
                ...
                <h4 class="text-xl font-semibold mb-2 relative z-10 text-slate-800">Perbarui Staf BK</h4>
                <p class="text-slate-400 text-xs font-medium mb-8 relative z-10">Konfirmasi semua perubahan data akun Guru BK.</p>
                ...
                <div class="w-full space-y-3 relative z-10">
                    ...
                    <a href="{{ route('admin.teachers.index') }}" class="block w-full bg-slate-50 hover:bg-slate-100 text-slate-500 font-bold py-3 rounded-lg transition text-[9px] border border-slate-100">
                        Batal
                    </a>
                </div>
            </div>
