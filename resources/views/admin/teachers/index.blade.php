...
<div class="max-w-6xl mx-auto space-y-8">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h2 class="text-3xl font-black text-slate-800 tracking-tight">Master Data Guru BK</h2>
            <p class="text-slate-500 font-medium">Kelola data guru bimbingan konseling dan kuota bimbingan siswa.</p>
        </div>
        <a href="{{ route('admin.teachers.create') }}" class="mt-4 md:mt-0 bg-primary hover:bg-secondary text-white font-black px-6 py-3 rounded-2xl shadow-lg shadow-primary/20 transition-all hover:scale-105 active:scale-95 flex items-center gap-2">
            ...
            <table class="w-full text-sm text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/30">
                        <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">Nama Guru</th>
                    ...
                    @forelse($teachers as $teacher)
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="px-8 py-6 text-slate-500 font-medium">
                                {{ $teacher->nip ?? '-' }}
                                <div class="text-[9px] text-slate-400 font-black uppercase tracking-tighter mt-1">NIP Resmi</div>
                            </td>
                            <td class="px-8 py-6 text-center">
                                <span class="font-black text-lg {{ $teacher->students_count >= $teacher->max_quota ? 'text-rose-600' : 'text-slate-800' }}">
                            ...
                            <td class="px-8 py-6 text-center">
                                ...
                                @if($sisa <= 0)
                                    <span class="bg-rose-50 text-rose-600 text-[10px] px-3 py-1.5 rounded-xl font-black uppercase tracking-widest border border-rose-100">Penuh</span>
                                @else
                        ...
                        <tr>
                            <td colspan="4" class="px-8 py-20 text-center">
                                <div class="flex flex-col items-center justify-center space-y-4 opacity-30">
                                    ...
                                    <p class="font-black uppercase tracking-widest text-sm">Belum ada data Guru BK</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
