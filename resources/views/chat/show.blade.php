    ...
    <div class="h-16 md:h-20 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 shrink-0 shadow-sm z-10">
        <div class="flex items-center gap-3 md:gap-5 min-w-0">
            ...
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    <h3 class="font-semibold text-slate-800 tracking-tight text-sm md:text-lg leading-tight truncate">{{ $other_user->name }}</h3>
                    <div class="flex items-center gap-1.5 mt-0.5">
                        <span class="relative flex h-1.5 w-1.5 shrink-0">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-500"></span>
                        </span>
                        <span class="text-[8px] md:text-[10px] font-bold text-slate-400 font-medium truncate">Live</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="hidden md:flex items-center gap-6">
            <div class="text-right">
                <div class="text-[10px] font-semibold text-slate-400 font-medium mb-0.5">Topik Konsultasi</div>
    ...
    <div class="flex-1 overflow-y-auto px-4 md:px-12 py-6 md:py-10 space-y-6 md:space-y-8 bg-[#f0f2f5] custom-scrollbar pattern-bg" id="chat-messages">
        ...
        <div class="max-w-2xl mx-auto mb-6 md:mb-10">
            <div class="bg-amber-50/80 backdrop-blur border border-amber-200/50 p-4 md:p-6 rounded-lg md:rounded-[2rem] flex items-start gap-3 md:gap-4 shadow-sm">
                ...
                <div>
                    <h4 class="text-xs md:text-sm font-semibold text-amber-900 uppercase tracking-tight">Mode Self-Destruct Aktif</h4>
        ...
        @empty
            <div class="flex flex-col items-center justify-center h-full space-y-4 opacity-30">
                ...
                <div class="text-center">
                    <p class="font-semibold uppercase tracking-[0.3em] text-xs md:text-sm text-slate-800">Mulai Percakapan</p>
                    <p class="text-[8px] md:text-[10px] font-bold mt-1 font-medium text-slate-500">Sampaikan keluhan atau pertanyaan Anda</p>
                </div>
            </div>
        @endforelse
    </div>
