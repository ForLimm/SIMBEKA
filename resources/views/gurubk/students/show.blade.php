...
<div class="w-full space-y-8" x-data="{ showExport: false }">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between bg-white p-6 rounded-3xl border border-slate-100 shadow-sm gap-6">
        <div class="flex items-center gap-6">
            ...
            <div>
                <h2 class="text-2xl font-black text-slate-800 tracking-tight leading-none">{{ $student->name }}</h2>
                <p class="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-2">Siswa Binaan: {{ $student->teacher->user->name ?? 'Belum Ditentukan' }}</p>
            </div>
        </div>
        <div class="flex flex-wrap items-center gap-3">
            ...
            <button @click="showExport = true" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-6 py-3 rounded-2xl transition shadow-lg shadow-indigo-600/20 flex items-center gap-2 text-sm">
        ...
        <div @click.away="showExport = false" class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-300">
            <div class="bg-indigo-600 p-8 text-white flex items-center justify-between relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <h3 class="font-black italic uppercase tracking-widest text-sm">Cetak Laporan Anekdot</h3>
                    <p class="text-indigo-100 text-[10px] font-bold uppercase tracking-widest mt-1 opacity-80">{{ $student->name }}</p>
                </div>
            ...
            <div class="card-premium p-8 space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-xs font-black text-slate-700">{{ $student->birth_date ? \Carbon\Carbon::parse($student->birth_date)->translatedFormat('d M Y') : '-' }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        ...
                        <div class="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                            ...
                            <div class="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] card-premium p-6 hover:shadow-xl transition-all">
                                <div class="flex items-center justify-between space-x-2 mb-2">
                                    <div class="font-black text-slate-800">{{ $session->counseling_date->translatedFormat('d M Y') }}</div>
                                ...
                                <div class="flex items-center justify-between pt-4 border-t border-slate-50">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($session->status === 'selesai')
                                            <a href="{{ route('gurubk.counseling.show', $session->id) }}" class="px-2 py-0.5 border border-slate-200 hover:border-primary hover:text-primary text-[9px] font-black uppercase rounded transition-all bg-white text-slate-500">
                                                Detail
                                            </a>
                                        @else
                                            <a href="{{ route('gurubk.counseling.edit', $session->id) }}" class="px-2 py-0.5 border border-slate-200 hover:border-primary hover:text-primary text-[9px] font-black uppercase rounded transition-all bg-white text-slate-500">
                                                Edit
                                            </a>
                                        @endif
                                    </div>
