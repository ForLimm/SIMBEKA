...
<div class="max-w-6xl mx-auto space-y-8" x-data="{ showExport: false, exportMode: 'all', selectedStudents: [], selectAll: false, toggleAll() { if(this.selectAll) { this.selectedStudents = [{!! $students->pluck('id')->map(fn($id) => '\'' . $id . '\'')->join(',') !!}]; } else { this.selectedStudents = []; } } }">
    ...
    <div class="flex items-center justify-between mb-8">
        ...
        <div class="flex items-center gap-3">
            <button @click="exportMode = 'all'; showExport = true" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-black px-6 py-3 rounded-2xl shadow-lg shadow-indigo-600/20 transition-all hover:scale-105 active:scale-95">
        ...
        <div @click.away="showExport = false" class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-300">
            <div class="bg-indigo-600 p-8 text-white flex items-center justify-between relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <h3 class="font-black italic uppercase tracking-widest text-sm" x-text="exportMode === 'all' ? 'Cetak Anekdot Keseluruhan' : (selectedStudents.length === 1 ? 'Cetak Anekdot Individu' : 'Cetak Anekdot Terpilih')">Cetak Anekdot Keseluruhan</h3>
                    <p class="text-indigo-100 text-[10px] font-bold uppercase tracking-widest mt-1 opacity-80" x-text="exportMode === 'all' ? 'Semua Anak Wali BK' : selectedStudents.length + ' Siswa Terpilih'">Semua Anak Wali BK</p>
                </div>
            ...
            <form action="{{ route('gurubk.anecdotes.export') }}" method="GET" class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="col-span-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-4 ml-1">Periode Laporan</label>
                        <div class="grid grid-cols-2 gap-3">
                            <label class="relative">
                                <input type="radio" name="period" value="month" class="peer sr-only">
                                <div class="p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 text-center cursor-pointer peer-checked:border-indigo-600 peer-checked:bg-indigo-50 transition-all">
                                    <span class="block text-[10px] font-black text-slate-800 peer-checked:text-indigo-700 uppercase tracking-widest">Bulan Ini</span>
                                </div>
                            </label>
                            <label class="relative">
                                <input type="radio" name="period" value="semester" class="peer sr-only">
                                <div class="p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 text-center cursor-pointer peer-checked:border-indigo-600 peer-checked:bg-indigo-50 transition-all">
                                    <span class="block text-[10px] font-black text-slate-800 peer-checked:text-indigo-700 uppercase tracking-widest">Semester Ini</span>
                                </div>
                            </label>
                            <label class="relative">
                                <input type="radio" name="period" value="year" class="peer sr-only">
                                <div class="p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 text-center cursor-pointer peer-checked:border-indigo-600 peer-checked:bg-indigo-50 transition-all">
                                    <span class="block text-[10px] font-black text-slate-800 peer-checked:text-indigo-700 uppercase tracking-widest">Tahun Ini</span>
                                </div>
                            </label>
                            <label class="relative">
                                <input type="radio" name="period" value="all" checked class="peer sr-only">
                                <div class="p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 text-center cursor-pointer peer-checked:border-indigo-600 peer-checked:bg-indigo-50 transition-all">
                                    <span class="block text-[10px] font-black text-slate-800 peer-checked:text-indigo-700 uppercase tracking-widest">Semua Data</span>
                                </div>
                            </label>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    <label class="relative col-span-3">
                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-4 ml-1">Format Dokumen</span>
                    </label>
                    <label class="relative">
                        <input type="radio" name="format" value="word" checked class="peer sr-only">
                        <div class="p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 text-center cursor-pointer peer-checked:border-indigo-600 peer-checked:bg-indigo-50 transition-all">
                            <span class="block text-[10px] font-black text-slate-800 peer-checked:text-indigo-700 uppercase tracking-widest">Word</span>
                        </div>
                    </label>
                    <label class="relative">
                        <input type="radio" name="format" value="pdf" class="peer sr-only">
                        <div class="p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 text-center cursor-pointer peer-checked:border-indigo-600 peer-checked:bg-indigo-50 transition-all">
                            <span class="block text-[10px] font-black text-slate-800 peer-checked:text-indigo-700 uppercase tracking-widest">PDF</span>
                        </div>
                    </label>
                    <label class="relative">
                        <input type="radio" name="format" value="excel" class="peer sr-only">
                        <div class="p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 text-center cursor-pointer peer-checked:border-indigo-600 peer-checked:bg-indigo-50 transition-all">
                            <span class="block text-[10px] font-black text-slate-800 peer-checked:text-indigo-700 uppercase tracking-widest">Excel</span>
                        </div>
                    </label>
                </div>

                <div class="pt-4">
                    <button type="submit" @click="setTimeout(() => showExport = false, 500)" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-[1.5rem] shadow-xl shadow-indigo-600/20 transition-all active:scale-[0.95] uppercase tracking-widest text-xs flex items-center justify-center gap-3">
    ...
    <div class="fixed bottom-8 left-0 right-0 flex justify-center z-50 pointer-events-none">
        <div x-show="selectedStudents.length > 0" 
            ...
            <div class="flex items-center gap-2 border-r border-slate-700 pr-6">
                <span class="w-6 h-6 bg-primary rounded-full flex items-center justify-center text-xs font-black" x-text="selectedStudents.length"></span>
                <span class="text-sm font-bold tracking-wide hidden sm:block">Siswa Terpilih</span>
            </div>
            <div class="flex items-center gap-3">
                <button type="button" @click="exportMode = 'selected'; showExport = true" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-600 rounded-xl text-xs font-bold transition flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                ...
                <form action="{{ route('gurubk.students.bulk_destroy') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus siswa yang dipilih secara permanen?')" class="inline">
                    ...
                    @method('DELETE')
                    <input type="hidden" name="student_ids" :value="selectedStudents.join(',')">
                    <button type="submit" class="px-4 py-2 bg-rose-500 hover:bg-rose-600 rounded-xl text-xs font-bold transition flex items-center gap-2">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50">
                        ...
                        <th class="px-4 py-5 text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">Info Siswa</th>
                        <th class="px-4 py-5 text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">NISN</th>
                        <th class="px-4 py-5 text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">Kelas</th>
                        <th class="px-4 py-5 text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right">Aksi</th>
                    </tr>
                </thead>
