...
<div class="w-full space-y-6">
    ...
    <div class="flex items-center justify-between bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        <div class="flex items-center gap-6">
            <a href="{{ route('gurubk.archives.index', ['type' => 'konseling']) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-50 text-slate-600 font-bold hover:bg-slate-100 transition shadow-sm text-xs group">
                <svg class="w-4 h-4 group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                Kembali
            </a>
            <div class="h-8 w-px bg-slate-100"></div>
            <div>
                <h2 class="text-2xl font-black text-slate-800 tracking-tight leading-none">Detail Sesi Bimbingan</h2>
                <p class="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-2">Catatan Konseling Bimbingan & Konseling</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">ID Sesi:</span>
            <span class="text-xs font-black text-slate-900 tracking-tighter">#{{ str_pad($session->id, 5, '0', STR_PAD_LEFT) }}</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            <div class="card-premium p-8">
                <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 border-b border-slate-100 pb-2">Ringkasan Sesi Konseling</h4>
                ...
                @if($session->follow_up)
                    <div class="mt-8 pt-8 border-t border-slate-100">
                        <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Rencana Tindak Lanjut</h4>
        ...
        <div class="space-y-6">
            ...
            <div class="card-premium p-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="font-black text-slate-900 tracking-tight">{{ $session->student->name }}</div>
                        @php
                ...
                <div class="mt-6 pt-6 border-t border-slate-100 space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs font-black text-slate-800 tracking-wider">{{ $session->student->nisn }}</p>
                    </div>
                    @endif
                    ...
                    <div>
                        ...
                        <p class="mt-1">
                            <span class="px-2.5 py-0.5 bg-slate-100 text-slate-600 text-[9px] font-black uppercase rounded">
                                {{ $session->category }}
                            </span>
                        </p>
                    </div>
