        ...
        <div class="xl:col-span-8 space-y-4">
            <div class="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden bg-white">
                <div class="bg-slate-50 px-8 py-4 border-b border-slate-100 flex items-center justify-between">
                    <h3 class="text-[10px] font-semibold text-slate-400 ">Pembaruan Catatan Sesi</h3>
                </div>
                
                <div class="p-8 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-[9px] font-semibold text-slate-400 font-medium mb-1.5 ml-1">Siswa Terkait <span class="text-rose-500">*</span></label>
                        ...
                        <div>
                            ...
                            <input type="date" name="counseling_date" value="{{ $session->counseling_date->format('Y-m-d') }}" required class="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition font-medium">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $session->title) }}" required class="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition font-medium" placeholder="Contoh: Motivasi Belajar Menurun / Pelanggaran Disiplin">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="summary" rows="6" required class="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-4 text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition font-medium italic resize-none" placeholder="Tuliskan inti dari sesi konseling yang telah dilakukan...">{{ $session->summary }}</textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden bg-white">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="follow_up" rows="4" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition resize-none font-medium" placeholder="Apa langkah selanjutnya? (Opsional)">{{ $session->follow_up }}</textarea>
                    </div>
                </div>
            </div>
