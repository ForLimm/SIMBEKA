...
<div class="w-full space-y-4">
    ...
    <div class="flex items-center justify-between bg-white p-6 rounded-lg border border-slate-100 shadow-sm">
        <div class="flex items-center gap-6">
            ...
            <div>
                <h2 class="text-2xl font-semibold text-slate-800 tracking-tight leading-none">Surat Peringatan (SP1)</h2>
                <p class="text-slate-400 text-xs text-slate-500 font-medium mt-2">Generate surat peringatan pertama resmi pelanggaran tata tertib</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
        <div class="bg-amber-600 px-8 py-6 text-white">
            ...
            <p class="text-amber-100 text-[10px] font-medium opacity-80 mt-1">Sistem Informasi Manajemen Bimbingan & Konseling</p>
        </div>
        ...
        <form action="{{ route('gurubk.letters.sp1.store') }}" method="POST" class="p-8 space-y-6">
            ...
            <div>
                ...
                <div class="relative">
                    <select name="student_id" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-5 py-4 text-sm focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 outline-none transition appearance-none font-medium" required>
            ...
            <div>
                ...
                <input type="text" name="letter_number" placeholder="Contoh: 421.7 / 001 / SMP.06 / {{ date('Y') }}" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-5 py-4 text-sm focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 outline-none transition font-medium" required>
            ...
            <div>
                <label class="block text-xs font-bold text-slate-400 font-medium mb-2 ml-1">Pelanggaran / Alasan SP1 <span class="text-rose-500">*</span></label>
                <textarea name="reason" rows="4" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-5 py-4 text-sm focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 outline-none transition resize-none font-medium" placeholder="Contoh: Sering bolos sekolah tanpa keterangan dan terlambat masuk..." required></textarea>
            </div>

            <div class="pt-6 border-t border-slate-100 flex justify-end gap-4">
                <button type="submit" class="w-full bg-amber-600 hover:bg-amber-700 text-white font-semibold py-4 rounded-lg shadow-xl shadow-amber-600/20 transition-all hover:scale-[1.02] active:scale-[0.95] flex items-center justify-center gap-2">
